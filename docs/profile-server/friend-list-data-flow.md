...
| `polcore+0x405820`| 0x28   | Friend status table (64 entries x 0x28) -- written by CallerB; read by WhoIs to compute subindex |
| `polcore+0x405800`| 0x28   | Adjacent table for sub-entry status flags             |
| `polcore+0x3F06CC`| 0x3098 | World list (4 worlds x 0x3098) -- written by `FUN_045AA520` from `(4,5,0x28)` SM result globals; consumed by world-list / search UI, NOT by `/flist` |
...
- Server pushes `N x 0xA8`-byte records. Polcore writes them DIRECTLY to Array2 via `FUN_0459EEB0`. After completion, the bitmap at `DAT_0463CA58` is finalized and FFXi handoff (`FUN_045A34F0`) fires.
...
- On state change (compared against current globals), calls `FUN_045AA520(acct_index, zone_id, world_index)` which updates the world list at `+0x3F06CC` (4 worlds x 0x3098 stride)
