...
[5]  uint32  body text ptr (NULL in observed rows; body fetched lazily on click)
...
| `DAT_0462A980` | global | queue base (4 entries × 0x40 bytes) |
...
### Friend server contract for click-to-read

When polcore connects (Init → Ack → Auth(3,1) → 0x198-byte request):
1. Parse the 0x17F-byte filename data at request offset 0x10 to identify which message body the client wants.
2. Look up the body in DB.
3. Respond with 4-byte total-size header.
4. Stream body bytes (BF-OFB encrypted with the same per-session key).
5. After all bytes sent, polcore enqueues type-3 notification → FFXi's callback fires → mes2frnd opens with the body data.
...
4. The per-frame friend driver `friend_per_frame (FFXi+0x102B20)` eventually fires the registered callback. Its inner block:
   ...
   if (state[+0x1050] != 0) {
       ...
       ...iterate friend lists (FFXi+0x91F949 ×100, FFXi+0x91F933 ×200)
